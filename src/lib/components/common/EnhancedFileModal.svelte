<script lang="ts">
	...
	const textExts = new Set([
		'txt', 'log', 'md', 'markdown', 'mdx', 'csv', 'tsv', 'json', 'jsonc', 'jsonl', 'json5',
		'html', 'htm', 'xml', 'yaml', 'yml', 'toml', 'ini', 'env', 'py', 'js', 'ts', 'tsx', 'jsx',
		'java', 'kt', 'kts', 'c', 'h', 'cpp', 'hpp', 'cs', 'go', 'rs', 'php', 'rb', 'sh', 'bash',
		'zsh', 'fish', 'sql', 'css', 'scss', 'less', 'svelte', 'vue', 'ipynb'
	]);
	...
	async function loadPreview(nextKey: string) {
		...
		clearPreview();
		try {
			rawData = await getFileContentById(item.id);
			if (isModel3dFile(filename)) {
				modelData = rawData;
			} else if (mime.startsWith('image/') || imageExts.has(ext)) {
				imageUrl = makeUrl(rawData, mime.startsWith('image/') ? mime : `image/${ext === 'svg' ? 'svg+xml' : ext}`);
			} else if (mime.startsWith('video/') || videoExts.has(ext)) {
				videoUrl = makeUrl(rawData, mime.startsWith('video/') ? mime : `video/${ext}`);
			} else if (mime.startsWith('audio/') || audioExts.has(ext)) {
				audioUrl = makeUrl(rawData, mime.startsWith('audio/') ? mime : `audio/${ext}`);
				...
				excelWorkbook = XLSX.read(new Uint8Array(rawData), { type: 'array' });
				...
				officeSlides = (await pptxToImages(rawData)).images;
			} else if (mime.startsWith('text/') || textExts.has(ext)) {
				textContent = new TextDecoder('utf-8', { fatal: false }).decode(rawData);
			}
		} catch (e) {
...
<Modal bind:show size="lg">
	<div class="flex h-[78vh] min-h-[28rem] w-full flex-col overflow-hidden p-3 text-gray-700 dark:text-gray-200">
		<div class="flex h-10 shrink-0 items-center justify-between gap-3 px-1 pb-2">
			...
			<div class="flex shrink-0 items-center gap-1.5">
				<button type="button" class="rounded-lg border border-gray-200 px-2.5 py-1 text-xs hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800" on:click={download}>{$i18n.t('Download')}</button>
				<button type="button" class="flex size-7 items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800" on:click={() => (show = false)} aria-label={$i18n.t('Close')}><XMark className="size-4" /></button>
			</div>
		</div>
		...
		<div class="custom-file-preview min-h-0 flex-1 overflow-hidden rounded-xl">
			...
			{:else if error}
				<div class="flex h-full items-center justify-center p-8 text-center text-sm text-red-500">{error}</div>
			...
			{:else if imageUrl !== null || videoUrl !== null || audioUrl !== null || pdfData !== null || sqliteData !== null || docxData !== null || textContent !== null || officeHtml !== null || officeSlides !== null}
				...
				/>
			{:else}
				<div class="flex h-full flex-col items-center justify-center gap-2 p-8 text-center text-sm text-gray-500">
					<div>{$i18n.t('No preview available for this file type.')}</div>
					<button type="button" class="rounded-lg bg-black px-3 py-1.5 text-xs text-white dark:bg-white dark:text-black" on:click={download}>{$i18n.t('Download')}</button>
				</div>
			{/if}
		</div>
	</div>
